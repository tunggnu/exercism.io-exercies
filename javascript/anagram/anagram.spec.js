...
describe('Anagram', () => {
  ...
  test('detects anagrams with Unicode and normal characters', () => {
    const subject = new Anagram('😅 😆 😉 😊');
    const matches = subject.matches(['cashreg', '😅😆😉😊   ', '😅  😆 😉😊']);

    expect(matches).toEqual(['😅😆😉😊   ', '😅  😆 😉😊']);
  });

  test('detects anagrams with Unicode characters only', () => {
    const subject = new Anagram('😅😆😉😊');
    const matches = subject.matches(['😅😆😊😉', 'Cart', '😉😊😅😆']);
